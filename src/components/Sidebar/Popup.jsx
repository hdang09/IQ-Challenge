...
const Popup = ({ open = false, handleClose = () => {} }) => {
    ...
    const handleSubmit = () => {
        ...
        const randomQuestionKit = JSON.parse(localStorage.getItem('question_kit'));
        // console.log(data.data.user[randomQuestionKit].questions[idx].answer);
        let correctAns = data.data.user[randomQuestionKit].questions.map((item, idx) => item.answer);
        let count = 0;
        for (let i = 0; i < correctAns.length; i++) {
            if (correctAns[i] === answers[i]) count++;
        }
